...
def gen_mwn2(osys,total_well,pumping_rate,nw_lay,nw_row,nw_col):
    ...
    fid = open('fw3.tmp', 'w')
    fid.write('%d %4d %4d %4.2f %s \n' %(nw_lay,nw_row,nw_col,pumping_rate,' 0 1.0 3745'))  # Write another line
    fid.close()

    # Combine all files
    if osys == 1:  # for Windows
        os.system('copy fw1.tmp + fw2.inp + fw3.tmp + fw4.inp TR_NWT_forward.wel')
        os.system('del *.tmp')
    else:  # for Linux
        os.system('cat fw1.tmp fw2.inp fw3.tmp fw4.inp > TR_NWT_forward.wel')
        os.system('rm -f *.tmp')
...
total_well = 3745  # that means only one new well is added
...
#print ID_HRiv_cells
ID_HRiv_cells = [int(y-1) for y in id_tmp] # Convert to int. Minus 1 b/c python array 0
...
if run_MODFLOW==1:
	if osys == 1:  # for Windows
		os.system('MODFLOW-NWT_64.exe TR_NWT_forward.nam')
	else:  # for Linux
	    os.system('./mfnwt TR_NWT_forward.nam')
	    os.system('rm -f fort.*')
...
list_stress_periods = cbb.get_kstpkper()  # Get a list of unique stress periods and time steps in the file
...
#	print 'ts = ',ts
	...
	FRF = cbb.get_data(kstpkper=(0,ts), text='FLOW RIGHT FACE',full3D=True)[0]
	FFF = cbb.get_data(kstpkper=(0,ts), text='FLOW FRONT FACE',full3D=True)[0]
	FLF = cbb.get_data(kstpkper=(0,ts), text='FLOW LOWER FACE',full3D=True)[0]
	...
	HDB = cbb.get_data(kstpkper=(0,ts), text='HEAD DEP BOUNDS',full3D=True)[0]
...
#		fid.write('%5d %4d %4d %4d %9.2f %9.2f %9.2f %9.2f %9.2f %9.2f %9.2f %9.2f %9.2f %9.2f\n' %(i,irow+1,icol+1,ilay+1,Right,Left,Front,Back,Lower,RLKi,HDBi,RCHi,WELi,Qkti))
	...
	Q_ORiv = QktEachFeature_tmp.sum() - Q_HRiv         # Sum of RLK from all other cells (not belong to the Humboldt river)
...
if well_withdrawl==0:
	np.savetxt('Qkt_before.dat', QktEachFeature_all_ts, delimiter=',',fmt='%9.5f')
